...
const SidebarContent: React.FC<SidebarContentProps> = ({
  ...
  const menuItems = [
    ...
    { label: "Add Appointment", path: "/add-appointment", icon: Plus },
    { label: "Providers", path: "/providers", icon: Users },
    { label: "Add Provider", path: "/add-provider", icon: Plus },
            ...
            return (
              <li className="mt-4" key={item.path}>
                <Link
                  to={item.path}
                  onClick={onClose}
                  className={`w-full flex items-center gap-4 px-4 py-2 rounded-xl transition-all duration-200 group ${
                    isActive
